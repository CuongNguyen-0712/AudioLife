    ...
    private void GenerateMapHtml(List<LocationPoint> locations, List<Category> categories, bool showRoute)
    {
        ...
        foreach (var point in locations)
        {
            ...
            markersJs.AppendLine(
                ...
                $"<div style=\"display:flex;align-items:center;gap:4px;margin-top:2px;font-size:10px;color:{onSurfaceVariant};\"><img src=\"{timeMetaIcon}\" style=\"width:12px;height:12px;flex:0 0 12px;opacity:0.95;\"/> <span>{loc.Duration} phút</span></div>" +
                $"<div style=\"display:flex;align-items:center;gap:4px;margin-top:4px;font-size:10px;color:{onSurfaceVariant};\"><span style=\"display:inline-flex;width:12px;height:12px;border-radius:999px;border:2px dashed {primary};box-sizing:border-box;\"></span> <span>Vùng kích hoạt {activationRadius} m</span></div></div>" +
