    ...
    private void GenerateMapHtml(List<LocationPoint> locations, List<Category> categories, bool showRoute)
    {
        ...
        foreach (var point in locations)
        {
            ...
            markersJs.AppendLine(
                ...
                $"<div style=\"display:flex;align-items:center;gap:4px;margin-top:4px;font-size:10px;color:{onSurfaceVariant};\"><img src=\"{audioMetaIcon}\" style=\"width:12px;height:12px;flex:0 0 12px;opacity:0.95;\"/> <span>{string.Format(T("Common_AudioCountFormat"), audioCount)}</span></div>" +
                $"<div style=\"display:flex;align-items:center;gap:4px;margin-top:2px;font-size:10px;color:{onSurfaceVariant};\"><img src=\"{timeMetaIcon}\" style=\"width:12px;height:12px;flex:0 0 12px;opacity:0.95;\"/> <span>{string.Format(T("Common_MinutesFormat"), loc.Duration)}</span></div></div>" +
